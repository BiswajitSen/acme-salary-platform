...
import {
  ALL_INSIGHT_QUERY_FILTER_DIMENSIONS,
  ALL_INSIGHT_QUERY_METRICS,
...
describe("query-spec", () => {
  ...
  it("defines filter support for every metric and dimension", () => {
    for (const metric of ALL_INSIGHT_QUERY_METRICS) {
      for (const dimension of ALL_INSIGHT_QUERY_FILTER_DIMENSIONS) {
        expect(typeof metricSupportsFilter(metric, dimension)).toBe("boolean");
      }
    }
  });
